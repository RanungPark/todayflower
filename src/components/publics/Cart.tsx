...
const Cart = ({open, setOpen}: CartProps) => {
  ...
  return (
    <CartWrapper>
      <CartHeaderWrapper>
        Shopping Cart
        <IconButton IconComponent={WClose} size="medium" onClick={handleClose} ariaLabel="close" />
      </CartHeaderWrapper>
      <CartMainWrapper>
        {carts.length === 0 ? <EmptyCartMessage>장바구니가 비어있습니다</EmptyCartMessage> : <CartItemCardList />}
      </CartMainWrapper>
      <CartfooterWrapper>
        <CartTotalPriceCard>Subtotal</CartTotalPriceCard>
        <ShippingNote>배송비 및 세금은 결제 시 계산됩니다. 오늘의 꽃 내 무료 표준 배송</ShippingNote>
