...
const TabletImgCarousel = ({products}: {products: ProductType[]}) => {
  const navigate = useNavigate();
  const [viewProducts, setViewProducts] = useState(products.filter(({id}) => id <= 3));

  const handleLeftButton = () => {
    if (viewProducts[0].id === 1) return null;
    else {
      const newViewProducts = products.filter(({id}) => viewProducts[0].id - 1 <= id && id <= viewProducts[2].id - 1);
      setViewProducts(newViewProducts);
    }
  };

  const handleRightButton = () => {
    if (viewProducts[2].id === products.length) return null;
    else {
      const newViewProducts = products.filter(({id}) => viewProducts[0].id + 1 <= id && id <= viewProducts[2].id + 1);
      setViewProducts(newViewProducts);
    }
...
export default TabletImgCarousel;
